...
function renderItems(h, _vm) {
  ...
  return itemsOptions ? itemsOptions.map(function (item) {
    ...
    if (item.itemRender && focusItemTypes.includes(item.itemRender.name) || !(item.itemRender && item.itemRender.name)) {
...
var _default2 = {
  ...
  data: function data() {
    return {
      ...
      currentColspan: 1,
      // 支持回车活动焦点的组件
      focusItemTypes: ["a-input", "a-password", "a-input-number", "a-select", "a-date-picker", "a-time-picker", "a-month-picker", "a-week-picker", "a-range-picker", "a-cascader", "a-tree-select", "a-textarea", "a-range-picker-split"],
      unifyApiGetOptions: [],
