...
var _defineProperty2 = _interopRequireDefault(require("E:/T/git/\u5965\u666ETeam/opu-components-vue/node_modules/@babel/runtime/helpers/esm/defineProperty"));
...
var _asyncToGenerator2 = _interopRequireDefault(require("E:/T/git/\u5965\u666ETeam/opu-components-vue/node_modules/@babel/runtime/helpers/esm/asyncToGenerator"));
...
var fetchItemPropsOptionsApiList = /*#__PURE__*/function () {
  var _ref = (0, _asyncToGenerator2.default)( /*#__PURE__*/regeneratorRuntime.mark(function _callee(list, _vm, formData, callback) {
    var setFieldsOptions, onOptionsAllLoad, onOptionsLoadBefore, onOptionsLoadAfter, autoSetDefaultValue, setFieldsOptionsDefaultValues, beforeRes, promises;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            setFieldsOptions = _vm.setFieldsOptions, onOptionsAllLoad = _vm.onOptionsAllLoad, onOptionsLoadBefore = _vm.onOptionsLoadBefore, onOptionsLoadAfter = _vm.onOptionsLoadAfter, autoSetDefaultValue = _vm.autoSetDefaultValue, setFieldsOptionsDefaultValues = _vm.setFieldsOptionsDefaultValues;
...
function renderItemInput(item, h, _vm) {
  ...
  if (item.itemRender && item.itemRender.slot) {
    ...
    // 根据name渲染组件
    var renderName = item.itemRender && item.itemRender.name && item.itemRender.name !== "hidden" ? "".concat(item.itemRender.name) : "a-input"; // if (renderName.indexOf("a-") > -1) {
    ...
    if (renderName.indexOf("a-") > -1) {
      ...
      configKey = _utils.default.lineToUpperCase(renderName, "-");
    }
      ...
      this.itemsOptions.forEach(function (item) {
        if (item && item.itemRender && item.itemRender.props && (fields.length && fields.includes(item.field) || fields.length === 0)) {
          ...
          if (input && input.getOptionsData) {
            ...
            var defaultRows = options.filter(function (p) {
              return p[defaultKey];
            });

            if (defaultRows.length > 0) {
              var defaultValue = defaultRows.map(function (p) {
                return p[valueField];
              });

              if (defaultValue.length) {
                var valueArrayTypes = ["a-checkbox-group", "a-radio-group"];
                var value = defaultValue;

                if (_utils.default.isArray(defaultValue)) {
                  var isSeletctMultiple = item.itemRender.name == "a-select" && item.itemRender.props && item.itemRender.props.mode == "multiple";
                  value = valueArrayTypes.includes(item.itemRender.name) || isSeletctMultiple ? defaultValue : defaultValue[0];
                }

                formData[item.field] = value;

                if (callback) {
                  formData = (0, _objectSpread2.default)((0, _objectSpread2.default)({}, formData), callback(item.field, defaultRows));
                }
              }
            }
          }
