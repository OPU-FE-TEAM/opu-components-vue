          ...
          case 8:
            ...
            Promise.all(promises).then(function (res) {
              ...
              list.forEach(function (item, index) {
                ...
                var itemData = _utils.default.getObjData(dataField, res[index]);

                if (fields && fields.length) {
                  // 统一请求可选数据 赋值到指定字段的处理
                  fields.forEach(function (element) {
                    var data = _utils.default.getObjData(element.dataField, itemData);
                  ...
                  var vF = "";
                  var lF = "";
                  vF = valueField ? valueField : _conf.default.getSelectOptions.valueField;
                  lF = labelField ? labelField : _conf.default.getSelectOptions.labelField; // }
...
function handeUnifyApiGetOptions(unifyList, optionsApiList, _vm) {
  ...
  unifyList.map(function (item) {
    ...
    var vF = valueField;
    var lF = labelField;
    var dF = dataField ? dataField : _conf.default.getSelectOptions.dataField; // if (["a-select", "a-checkbox-group"].includes(item.itemRender.name)) {
    ...
    cloneItems: function cloneItems(items) {
      ...
      var data = clone.map(function (item) {
        ...
        if (item.itemRender && item.itemRender.props && item.itemRender.props.options) {
          var vF = "";
          var lF = "";
          vF = item.itemRender.props.valueField ? item.itemRender.props.valueField : _conf.default.getSelectOptions.valueField;
          lF = item.itemRender.props.labelField ? item.itemRender.props.labelField : _conf.default.getSelectOptions.labelField;
          var arr = (0, _objectSpread2.default)({}, item.itemRender.props);

          if (!["a-cascader"].includes(item.itemRender.name)) {
            arr = (0, _objectSpread2.default)((0, _objectSpread2.default)({}, item.itemRender.props), {}, {
              valueField: vF,
              labelField: lF
            });
          }

          var options = handleItemPropsOptions(arr);
