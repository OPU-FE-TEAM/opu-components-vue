...
var fetchItemPropsOptionsApiList = /*#__PURE__*/function () {
  var _ref = (0, _asyncToGenerator2.default)( /*#__PURE__*/regeneratorRuntime.mark(function _callee(list, _vm, formData) {
  ...
  return function fetchItemPropsOptionsApiList(_x, _x2, _x3) {
...
var _default2 = {
  ...
  data: function data() {
    return {
      ...
      currentColspan: 1,
      // 支持回车活动焦点的组件
      focusItemTypes: ["a-input", "a-password", "a-input-number", "a-select", "a-date-picker", "a-time-picker", "a-month-picker", "a-week-picker", "a-range-picker", "a-cascader", "a-tree-select", "a-textarea", "a-range-picker-split", "a-input-number-split"],
      unifyApiGetOptions: [],
  ...
  computed: {
    ...
    enterToNextItemFocusList: function enterToNextItemFocusList() {
      ...
      return this.items.map(function (item) {
        if (item.itemRender && _this.focusItemTypes.includes(item.itemRender.name) && (!(item.itemRender.props && (item.itemRender.props.disabled == true || item.itemRender.props.readonly == true)) || !item.itemRender.props)) {
          // 可获得焦点的组件
          return item.field;
  ...
  methods: {
    ...
    loadOptionsData: function loadOptionsData() {
      ...
      if (unifyApiGetOptions.length) {
        handeUnifyApiGetOptions(unifyApiGetOptions, getItemPropsOptionsApiList, this, formData);
      } else if (getItemPropsOptionsApiList.length) {
        fetchItemPropsOptionsApiList(getItemPropsOptionsApiList, this, formData);
      }
    },
    // 获取表单数据，不验证
