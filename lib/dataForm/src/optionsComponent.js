...
function handleItemPropsOptions(options, valueField, labelField) {
  ...
  if ((valueField || labelField) && options && _utils.default.isArray(options)) {
    ...
    return cloneOptions.map(function (item) {
      ...
      if (item.children && item.children.length) {
        item.children = handleItemPropsOptions(item.children, valueField, labelField);
      }
  ...
  methods: {
    ...
    fetchOptionsData: function fetchOptionsData() {
      ...
      fetchApi(param).then(function (res) {
        ...
        var options = handleItemPropsOptions(data, vF, lF);
