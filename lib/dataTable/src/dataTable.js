    ...
    tableProps: function tableProps() {
      ...
      var columns = tableColumns.map(function (item) {
        ...
        if (item.editRender) {
          ...
          } else if (item.editRender.name && item.editRender.name === "ASwitch") {
            item.slots = {
              default: "a_switch",
              edit: "a_switch"
            };
            _this2.$scopedSlots["a_switch"] = renderSwitch;
          } else if (item.editRender.name && item.editRender.name === "pulldownTable") {
            item.slots = {
      ...
      if (proxyConfigOpt && proxyConfigOpt.ajax && proxyConfigOpt.ajax.query) {
        ...
        props.props.proxyConfig.ajax.query = function (arr) {
          ...
          return new Promise(function (resolve, reject) {
            proxyConfigOpt.ajax.query(arr).then(function (res) {
              resolve(res);
            }).catch(function (err) {
              reject(err);
            });
